...
if (isset($_POST['submit'])) {
    ...
    if ($textlist != "") {
        $sql = "INSERT INTO daftarkegiatan (idlist, kegiatan, status, idUser) VALUES (?, 'simpan', ?)";
        $stmt = $db->prepare($sql);
        $stmt->bind_param("si", $textlist, $ID);
        $stmt->execute();
    ...
    <section id='todo-page'>
        <div id="page">
            ...
            <div class="listdaftar">
                <?php
                if (isset($ID)) {
                    $sql = "SELECT idList, list, status FROM daftarlist WHERE idUser = '$ID'";
                ...
                ?>
                            <div class="cover" id="cover_<?php echo $row["idList"] ?>">
                                <?php
                                if ($row['status'] == 'simpan') {
                                ?>
                                    <div id="daftar"><?php echo $row["list"]; ?></div>
                                    <form action="index.php" method="post">
                                        <input type="hidden" name="listdel" value="<?php echo $row['idList']; ?>">
                ...
                if (isset($_POST['haps'])) {
                    ...
                    $sql = "DELETE FROM daftarlist WHERE idList = '$listDel'";
                    $result = $db->Query($sql);
                    header('Location: ' . $_SERVER['PHP_SELF']);
                    exit();
                }
                ...
                if (isset($_POST['selesai'])) {
                    ...
                    $sql = "UPDATE daftarlist SET status = 'Selesai' WHERE idList = '$listDel'";
                    $result = $db->Query($sql);
                    header('Location: ' . $_SERVER['PHP_SELF']);
                    exit();
                }
